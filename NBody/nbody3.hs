...
advance count dt nBodies nAxes masses positions velocities =
  let vecSize                 = nBodies * nAxes
      ...
      advanceVelsFn (b,b2) = let ob_0  = offset b 0
                                 ...
                                 -- GHC really likes this form for velForFn.
                                 velForFn oset = let (b3,a) = invOffset oset
                                                 in if b3 == b
                                                    then let vf (vels, dx, dy, dz, mag) = case a of
                                                                                               0 -> (vels U.! ob_0) - dx * mb2 * mag
                                                                                               1 -> (vels U.! ob_1) - dy * mb2 * mag
                                                                                               2 -> (vels U.! ob_2) - dz * mb2 * mag
                                                         in vf
                                                    else if b3 == b2
                                                    then let vf (vels, dx, dy, dz, mag) = case a of 
                                                                                               0 -> (vels U.! ob2_0) + dx * mb  * mag
                                                                                               1 -> (vels U.! ob2_1) + dy * mb  * mag
                                                                                               2 -> (vels U.! ob2_2) + dz * mb  * mag
                                                         in vf
                                                    else let vf (vels, dx, dy, dz, mag) = vels U.! oset
                                                         in vf 
                                 ...
                                 advanceVels' (posns, vels) = let dx       = (posns U.! ob_0) - (posns U.! ob2_0)
                                                              ...
                                                              in (posns, (U.convert $ V.map ($ (vels, dx, dy, dz, mag)) velForFns ) :: U.Vector(Double))
      ...
      advance' count positions velocities =
        ...
        in newPos `seq` (advance' (count-1) newPos newVels )
